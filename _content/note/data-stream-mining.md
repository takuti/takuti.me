...
「じゃあ何をスケッチしたいのか」という話になる。データ解析といっても得たい結果は様々で、特にストリーム上の近似的なデータ解析ともなるとあまり凝ったことはできない。
...
2013年のKDDベストペーパーになった手法 ***Frequent Directions*** は、ベクトル列なデータの基底をコンパクトなスケッチ行列で表現する手法。小さい行列に対して繰り返し特異値分解を行うアルゴリズムなので計算量はそれほど重くないし、行列の低ランク近似の文脈で誤差解析に関する知見は豊富にあるので議論がしやすい、というのがウリ。
...
iwiwiさんの資料。乱択スケッチという、データのスケッチの議論にランダム性を取り入れた考え方関する最新事情の紹介。グラフに対するスケッチのお話は知見がなかったので新鮮だった。
...
サミット冒頭の『すごいリンク集』で紹介した[Muthuさん](http://www.cs.rutgers.edu/~muthu/)も登壇している。
...
ストリームデータを捌く基盤として **[Fluentd](http://www.fluentd.org/)** が考えられるし、[合わせて使われる](http://docs.fluentd.org/articles/cep-norikra)ストリーム処理サーバ **[Norikra](http://norikra.github.io/)** の土台となっているCEP (Complex Event Processing) はアカデミックで議論されてきた考え方だ。
...
機械学習・データマイニング系のトップ会議ではここ数年 "Data Streams" というセッションが設けられることも珍しくなく、基礎と応用の狭間を生きる人にとっては良い選択肢なのではないかと感じる。
